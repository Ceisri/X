...
func saveData() -> void:
	...
	var file = File.new()
	var path = SAVE_DIR + player.entity_name + "_equipment.save"
	...
	var buttonPath = "user://button_list.save"
	var buttonFile = File.new()
	...
	if buttonFile.file_exists(buttonPath):
		if buttonFile.open(buttonPath, File.READ) == OK:
			var loadedData = buttonFile.get_var()
			buttonFile.close()

			if typeof(loadedData) == TYPE_DICTIONARY:
				buttonData = loadedData

	if !buttonData.has("buttons") or typeof(buttonData["buttons"]) != TYPE_ARRAY:
		buttonData["buttons"] = []

	if !buttonData.has("sexes") or typeof(buttonData["sexes"]) != TYPE_DICTIONARY:
		buttonData["sexes"] = {}

	if !buttonData.has("equipment") or typeof(buttonData["equipment"]) != TYPE_DICTIONARY:
		buttonData["equipment"] = {}

	buttonData["equipment"][player.entity_name] = {
		"torso": current_torso_scene.resource_path if current_torso_scene else "",
		"hands": current_hands_scene.resource_path if current_hands_scene else "",
		"feet": current_feet_scene.resource_path if current_feet_scene else ""
	}

	if buttonFile.open(buttonPath, File.WRITE) == OK:
		buttonFile.store_var(buttonData)
		buttonFile.close()
#func saveData() -> void:
#	player.entity_name 
#	var dir=Directory.new()
